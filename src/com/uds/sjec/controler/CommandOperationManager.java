...
public class CommandOperationManager extends BaseModuleOperation {
	...
	protected void DoUserTask() {
		...
		try {
			...
			if ("com.uds.sjec.commands.batchExportExcel".equals(m_commandId)) {// 一级BOM生成excel
			...
			if ("com.uds.sjec.commands.EditVariableCondition".equals(m_commandId)) {// 编辑变量条件
				CommonFunction.Init();
				if (!ConstDefine.IFDEFINE) {
					return;
				}
				if (!ConstDefine.IFCANDO) {
					MessageBox.post("您没有权限进行该操作", "提示", MessageBox.INFORMATION);
				}
