...
public class CfgManagementFrame extends JFrame {
	/**
	 * 
	 */
	...
	public CfgManagementFrame() {
		...
		button_BOMConfiguration.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (productType != null) {
					TCComponentQuery query = QueryUtil.getTCComponentQuery("Item...");
					TCComponent[] searchResult = QueryUtil.getSearchResult(query, new String[] { "零组件 ID" }, new String[] { productType + ConstDefine.TC_GROUP_SUFFIX });
					if (searchResult.length > 0) {
